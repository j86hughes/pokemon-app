...
const Stats = ({ pokemonItem }) => {
  ...
  const statsMapped = statsArray.map((stats) => {``
    const statName = stats.stat.name.charAt(0) + stats.stat.name.slice(1, 8).replace('-', ' ') + stats.stat.name.charAt(8) + stats.stat.name.slice(9)
            ...
            {numsArray.map((num, index) => {
              return (
                <div className='stat-ul-div'>
                  <StatBar numOfBarsColored={num} statName={statsMapped[index]} />
                </div>
              );
            }
            )}
